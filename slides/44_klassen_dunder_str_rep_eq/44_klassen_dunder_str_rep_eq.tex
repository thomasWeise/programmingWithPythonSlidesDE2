...
Das Zielpublikum sind hier Programmier, die am Kode arbeiten, und die \DEzB\ genaue Informationen in Log-Dateien schreiben oder nach Fehlern suchen müssen.\uncover<8->{ %
...
%
\only<-23>{%
\item<20-> Wenn die textuelle Repräsentation der Standard-Kollektionstypen von \python\ mit \pythonilIdx{str} oder \pythonilIdx{repr} erzeugt wird, dann werden die Elemente der Kollektionen immer \pythonilIdx{repr} zu Strings konvertiert, nie mit~\pythonilIdx{str}\cite{PEP3140}.%
}%
%
\only<-25>{%
...
%
\only<-7>{%
\item<3-> Diese Klasse war ziemlich nützlich, als wir Klassen für verschiedene Formen implementiert hatten.%
}%
%
\only<-8>{%
...
%
\only<-32>{%
\item<28-> Das ist nicht immer der Fall, obwohl ich kein Beispiel kenne, wo es nicht simmt\cite{programmingWithPython} {\dots} vielleicht war es früher mal so, dass das bei Fließkommazahlen auftreten konnte\cite{PEP754}, im Moment sehe ich das aber nicht.%
}%
%
\only<-33>{%
