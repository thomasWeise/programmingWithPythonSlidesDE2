...
\begin{itemize}%
\only<-4>{%
\item Wir willen alle geschlossenen Formen auf der zweidimensionalen Euklidischen Ebene repräsentieren können.%
}%
%
\only<-5>{%
...
%
\only<-6>{%
\item<2-> Die Klasse ist nicht dafpr da, um wirklich instantiiert zu werden.%
}%
%
\only<-7>{%
...
%
\only<-5>{%
\item<2-> Ein Kreis hat einen Mittelpunkt~(center) und einen Radius.%
}%
%
\only<-6>{%
...
%
\only<-9>{%
\item<6-> Die Klasse \pythonil{Circle} hätte zwei Attributes.%
}%
%
\only<-10>{%
...
%
\only<-6>{%
\item<2-> Wir erstellen die Instance \pythonil{circ} of \pythonil{Circle} by providing the \pythonil{point=Point(2, 3)} and \pythonil{radius=4}.%
}%
%
\only<-7>{%
...
%
\only<-11>{%
\item<8-> Wir können also annehmen, dass alle nicht-abstrakten Subklassen von \pythonil{Polygon} immer die Methode \pythonil{points} vernünftig implementieren und diese die Sequenz von Eckpunkten des Polygons lifert.%
}%
%
\only<-13>{%
...
%
\only<-20>{%
\item<19-> Wenn wir ein Subklasse erstellen, die \pythonil{points} implementiert, und dann \pythonil{perimeter} von einem Objekt der Subklasse aufrufen, dann wird das funktionieren.%
}%
%
\only<-23>{%
...
%
\only<-16>{%
\item<13-> Die Höhe ist der Unterschied zwischen den x\nobreakdashes-Koordinaten der beiden Eckpunkte.%
}%
%
\only<-17>{%
...
\begin{itemize}%
\only<-10>{%
\item Wir haben nun diskutiert wie Veerbung mit Klassen in \python\ funktioniert.%
}%
%
\only<-11>{%
