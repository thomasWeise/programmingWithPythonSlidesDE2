...
%
\only<-24>{%
\item<21-> Die letzte Subtraktion von \pythonil{1e18} liefert dann \pythonil{-1e18}.%
}%
%
\only<-25>{%
...
%
\only<-14>{%
\item<14-> Natürlich wird diese auf die Attribute \pythonil{self.__sum}, \pythonil{self.__cs} und \pythonil{self.__ccs} anstelle von \aVar{sum}, \aVar{cs} und \aVar{ccs} zugreifen.%
}%
%
\only<-16>{%
...
%
\item<4-> Die Klasse erlaubt es uns, Zahlen präziser zu addieren, in dem sie intern Fehlerterme zusätzlich zur normalen Summe unterhält.%
%
